#!/bin/bash
# nurandom: NuSoftHEP per-event random management for art (cetmodules, art plugins).
# _CheckClassVersion off is the standard ROOT-dict guard (harmless if no dict).
set -euo pipefail
